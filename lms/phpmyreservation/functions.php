...
{
	$query = mysql_query("SELECT * FROM " . global_mysql_users_table . " WHERE username='$user_name'")or die('<span class="error_span"><u>MySQL error:</u> ' . htmlspecialchars(mysql_error()) . '</span>');
...
function get_login_data($data)
{
	...
	elseif($data == 'password' && isset($_COOKIE[global_cookie_prefix . '_user_password']))
	{
...
function login($user_email, $user_password, $user_remember)
{
	...
	$query = mysql_query("SELECT * FROM " . global_mysql_users_table . " WHERE user_email='$user_email' AND password='$user_password_encrypted' OR user_email='$user_email' AND password='$user_password'")or die('<span class="error_span"><u>MySQL error:</u> ' . htmlspecialchars(mysql_error()) . '</span>');
...
function create_user($user_name, $user_email, $user_password, $user_secret_code)
{
	...
	else
	{
		...
		mysql_query("INSERT INTO " . global_mysql_users_table . " (user_is_admin,user_email,password,username,user_reservation_reminder) VALUES ($user_is_admin,'$user_email','$user_password','$user_name','0')")or die('<span class="error_span"><u>MySQL error:</u> ' . htmlspecialchars(mysql_error()) . '</span>');
...
function list_admin_users()
{
	...
	else
	{
		...
		while($user = mysql_fetch_array($query))
		{
			...
			$return .= '<tr><td>' . $user['username'] . '</td><td><span id="email_span_' . $i . '"></span></td></tr><script type="text/javascript">$(\'#email_span_' . $i . '\').html(\'<a href="mailto:\'+$.base64.decode(\'' . base64_encode($user['user_email']) . '\')+\'">\'+$.base64.decode(\'' . base64_encode($user['user_email']) . '\')+\'</a>\');</script>';
		}
...
function list_users()
{
	$query = mysql_query("SELECT * FROM " . global_mysql_users_table . " ORDER BY user_is_admin DESC, username")or die('<span class="error_span"><u>MySQL error:</u> ' . htmlspecialchars(mysql_error()) . '</span>');
...
function reset_user_password($user_id)
{
	...
	mysql_query("UPDATE " . global_mysql_users_table . " SET password='$password_encrypted' WHERE user_id='$user_id'")or die('<span class="error_span"><u>MySQL error:</u> ' . htmlspecialchars(mysql_error()) . '</span>');
...
function change_user_details($user_name, $user_email, $user_password)
{
	...
	else
	{
		...
		{
			mysql_query("UPDATE " . global_mysql_users_table . " SET username='$user_name', user_email='$user_email' WHERE user_id='$user_id'")or die('<span class="error_span"><u>MySQL error:</u> ' . htmlspecialchars(mysql_error()) . '</span>');
		}
		else
		{
			$user_password = encrypt_password($user_password);

			mysql_query("UPDATE " . global_mysql_users_table . " SET username='$user_name', user_email='$user_email', password='$user_password' WHERE user_id='$user_id'")or die('<span class="error_span"><u>MySQL error:</u> ' . htmlspecialchars(mysql_error()) . '</span>');
		}
